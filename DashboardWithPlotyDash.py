...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                #options=[{'label': 'Option 1', 'value': '1'}, {'label': 'Option 2', 'value': '2'}], value='1'
                                ...
                                html.Br(),
                                html.Div([
                                    html.P('This is a sample dashboard showing various Dash components.'),
                                    html.Hr()
                                ]),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        class_counts.columns = ['Success/Failure', 'Count']
        class_counts['Success/Failure'] = class_counts['Success/Failure'].map({1: 'Success', 0: 'Failure'})
...
def update_scatter(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(
            ...
            hover_data=['Launch Site', 'Booster Version', 'Booster Version Category']
        ...
        filtered_df_range = spacex_df[
            (spacex_df['Payload Mass (kg)'] >= payload_range[0]) &
            (spacex_df['Payload Mass (kg)'] <= payload_range[1])
            ]
        filtered_df = filtered_df_range[filtered_df_range['Launch Site'] == entered_site]
        fig = px.scatter(
            filtered_df,
            x='Payload Mass (kg)',
            y='class',
